annotate <- function(col=c("black","red","green","blue"), lwd=c(4,4,30), cex=c(1,1), info=TRUE) {
   ...
   colpos <- .info(mode, col, colnum, lwd.draw, lwd.eraser, lwd.symb, cex.pt, cex.txt, snap, smooth, info)
   ...
   fun.mousedown <- function(button,x,y) {
      ...
      if (info) {
         colsel <- apply(colpos, 1, function(pos) x > pos[1] && x < pos[3] && y > pos[2] && y < pos[4])
         if (any(colsel)) {
            colnum <<- which(colsel)
            colpos <<- .info(mode, col, colnum, lwd.draw, lwd.eraser, lwd.symb, cex.pt, cex.txt, snap, smooth, info)
            pressed <<- FALSE
      ...
      if (pressed) {
         ...
         if (mode == "draw") {
            if (!smooth)
               ...
               #lines(c(x.last, x), c(y.last, y), lwd=lwd.draw, col=col[colnum])
         }
         if (mode == "eraser")
            segments(x.last, y.last, x, y, lwd=lwd.eraser, col=col.bg)
            #lines(c(x.last, x), c(y.last, y), lwd=lwd.eraser, col=col.bg)
         x.last <<- x
         y.last <<- y
      }
      return(NULL)
   ...
   fun.key <- function(key) {
      ...
      if (mode == "type") {
         ...
         if (key == "\r" || key == "ctrl-J") {
            text(x.start, y.start, txt, cex=cex.txt, adj=c(0,0.5), col=col[colnum])
